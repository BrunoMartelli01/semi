...
class TextEvaluator():
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        assert(int(self.voc_size - 1) == len(self.CTLABELS)), \
            "voc_size is not matched dictionary size, got {} and {}.".format(int(self.voc_size - 1), len(self.CTLABELS))
        ...
        if "totaltext" in dataset_name:
            ...
            self._text_eval_gt_path = "datasets/evaluation/gt_hiertext.zip"
            self._word_spotting = False
        ...
        for i in files:
            if "ic15" in self.dataset_name:
                ...
                if self.lexicon_type == 3:
                    lexicon_path = 'datasets/ic15/new_strong_lexicon/new_voc_img_' + str(
                        int(i.split('/')[-1].split('.')[0])) + '.txt'
                    lexicon_fid = open(lexicon_path, 'r')
                    pair_list = open('datasets/ic15/new_strong_lexicon/pair_voc_img_' + str(
                        int(i.split('/')[-1].split('.')[0])) + '.txt')
                    pairs = dict()
                    for line in pair_list.readlines():
                        line = line.strip()
            ...
            for iline, line in enumerate(fin):
                ...
                for ipt in pts:
                    if "ctw1500" in self.dataset_name:
                        outstr += (str(float(ipt[0])) + ',' + str(float(ipt[1])) + ',')
    ...
    def evaluate_with_official_code(self, result_path, gt_path):
        if "ic15" in self.dataset_name:
            ...
            return text_eval_script_ct.text_eval_main_ct(det_file=result_path, gt_file=gt_path,
                                                          is_word_spotting=self._word_spotting)
        else:
    ...
    def evaluate(self):
        ...
        if (not 'textocr' in self.dataset_name) and \
           ...
           (not 'cocotext' in self.dataset_name) :
            ...
            text_result_full = self.evaluate_with_official_code(result_path_full, self._text_eval_gt_path)
            ...
            for k in range(2):
                ...
                for j in range(3):
                    ...
                    dict_lexicon = {"1": "Generic", "2": "Weak", "3": "Strong"}
                    text_result_full = self.evaluate_with_official_code(result_path_full, self._text_eval_gt_path)
                    text_result_full["e2e_method"] = f'{title}' + "-" + dict_lexicon[
                        str(self.lexicon_type)] + "-" + text_result_full["e2e_method"]
                    shutil.rmtree(temp_dir.split('/', 1)[0])
